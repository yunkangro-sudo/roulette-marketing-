...
### 추가
- 이벤트 도전 횟수 **N일 직접입력** (`custom` + `challenge_frequency_days`) — 기존 주간/월간 고정 옵션 제거, 관리자 UI `1일1회` / `(숫자)일` / `무제한`
- 업체정보 **네이버 후기쓰기 URL** / **구글 맵 후기쓰기 URL** — 입력 시 손님 쿠폰함에 버튼 동적 생성(당근·네이버·구글 후기 포함 최대 4개)
- 내 쿠폰함 **홈화면에 추가(PWA)** — 매장별 동적 manifest, 최소 서비스워커(캐싱 없음), 안드로이드/iOS/인앱/PC 분기, `lib/pwa/pwaInstall.ts`

### 변경
- 이벤트 도전 횟수 "매일" 라벨 → **1일1회**
- 안드로이드 PWA: `beforeinstallprompt` 미발생(크롬 참여도 휴리스틱) 시에도 버튼 노출 + 크롬 메뉴 수동 설치 안내 팝업

### 수정
- PWA 설치 리스너/서비스워커 등록이 로그인·데이터 로딩 완료 후에야 실행되던 타이밍 문제 수정 — 페이지 JS 로드 즉시 `pwaInstall.ts`에서 처리
