...
interface FunnelData {
  ...
  paymentTotal: number
  additionalRevenue: { value: number; isActual: boolean; sampleCount: number }
  roi: number | null
}
...
function FunnelRow({ step, label, value, sub, badge, highlight }: {
  step: number; label: string; value: string; sub?: string; badge?: React.ReactNode; highlight?: boolean
}) {
  return (
...
export default function ReportClient({ role, storeId }: ReportClientProps) {
  ...
  const couponUseRate = data && data.couponsIssued > 0
    ? Math.round((data.couponsUsed / data.couponsIssued) * 1000) / 10 : 0
        ...
        {loading ? (
          ...
          <>
            <div className="px-1 pb-1">
              ...
              <p className="text-xs text-gray-400 mt-0.5">평균 객단가 {(data.avgOrderValue ?? 0).toLocaleString()}원 기준</p>
            </div>

            <FunnelRow step={1} label="월 광고비" value={`${(data.adBudget ?? 0).toLocaleString()}원`} sub="store_settings 기준" />
            ...
            <div className="text-center text-gray-300 text-lg">↓</div>
            <FunnelRow step={4} label="쿠폰 사용" value={`${data.couponsUsed.toLocaleString()}건`}
            ...
            <div className="text-center text-gray-300 text-lg">↓</div>
            <FunnelRow step={6} label="추가 매출"
              value={`${(data.additionalRevenue.value ?? 0).toLocaleString()}원`}
              sub={data.additionalRevenue.isActual
                ? `결제금액 합계 (총 ${data.additionalRevenue.sampleCount}건)`
                : `객단가 ${(data.avgOrderValue ?? 0).toLocaleString()}원 × 재방문 ${data.returningVisitors}명`}
              badge={<Badge isActual={data.additionalRevenue.isActual} sampleCount={data.additionalRevenue.sampleCount} />}
            />
            <div className="text-center text-gray-300 text-lg">↓</div>

            <div className={`px-5 py-5 rounded-xl border-2 ${data.roi !== null && data.roi >= 1 ? 'bg-orange-500 border-orange-500' : 'bg-gray-100 border-gray-200'}`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm font-semibold ${data.roi !== null && data.roi >= 1 ? 'text-orange-100' : 'text-gray-500'}`}>
                    ROI (추가매출 ÷ 광고비)
                  </p>
                  {data.adBudget <= 0 && <p className="text-xs text-gray-400 mt-0.5">광고비 미입력 시 계산 불가</p>}
                </div>
                <p className={`text-4xl font-black ${data.roi !== null && data.roi >= 1 ? 'text-white' : 'text-gray-400'}`}>
