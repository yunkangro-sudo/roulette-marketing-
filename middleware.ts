 ...
 *
 * 1) 광고주(advertiser) 이용기간 만료 + 가입승인 게이트 차단
 *    - 오늘 <= end_date: 정상 이용
 *    - end_date < 오늘 <= end_date + 7일(유예): 정상 이용 (경고 배너는 layout에서 표시)
 *    - 오늘 > end_date + 7일: 지정 경로 전부 차단 → /admin/expired 로 이동
 *    - subscriptions row가 아예 없는 매장(= 'trial' = 승인대기, 신규 가입 직후)도 동일하게
 *      /admin/expired 로 차단한다. 회원가입(/api/signup) 완료 시점엔 subscriptions row가
 *      생성되지 않으므로, 슈퍼관리자가 "새 결제 등록"으로 입금 확인 후 최초 구독 row를
 *      넣어주기 전까지는 관리자 기능을 쓸 수 없다 (그 등록 행위 자체가 곧 "승인").
 ...
 *    취급하되, 이용기간 만료/승인대기로 인한 차단만은 적용하지 않는다 — 슈퍼관리자가
 *    만료되거나 승인대기 중인 업체를 들여다보고 처리해야 하는 상황이 오히려 더 흔하기
 *    때문 (배너로만 안내).
 */
export async function middleware(request: NextRequest) {
  ...
  if (rawAccount.impersonation) return response // 대리접속 중엔 만료/승인대기 차단 없음, 배너만

  const status = await getSubscriptionStatus(account.storeId)
  // 'trial' = subscriptions row가 아예 없는 승인대기 상태 — 'expired'와 같은 카테고리로 차단
  if (status.status === 'expired' || status.status === 'trial') {
