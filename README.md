...
- [x] **Phase 2: 샘플 매장 가상 활동데이터** (`scripts/seed-demo-activity.mjs`) — `upsert_customer_loyalty` RPC가 `now()` 고정이라 과거 날짜 시뮬레이션에 못 써서, `customer_loyalty`/`activity_log`/`point_ledger`/`coupons`를 직접 과거 날짜로 계산해 채우는 별도 스크립트로 구현. 기간은 8~12주→3~8주로 축소(사장님 확인). 재실행 시 기존 가상 데이터를 지우고 재생성(멱등)

### 2026-09-03 (랜딩페이지 세부 수정 + 매장 홈페이지 유료 애드온 게이팅 + 쿠폰함 바로가기)
- [x] 랜딩페이지 세부 수정 다수 — 데이터증명-계산기 섹션 전환 정리(모바일 계산기 상시노출), 성과리포트 이미지 교체, "우리 매장 홈페이지" 섹션 카피를 SEO·AEO 포지셔닝으로 개편, 요금제에 "첫 1년 유지비 무료" 프로모션 반영, 요금제 계산기를 팝업 모달로 분리
- [x] `docs/migrations/055_store_addons.sql` — **매장 홈페이지 기능 유료 애드온 게이팅** 신규 `store_addons`(`homepage_feature_enabled`) 테이블. 결제 게이트(애드온)가 꺼져있으면 광고주 본인의 `business_entity.homepage_enabled` 설정과 무관하게 무조건 "준비중" 화면으로 대체(상위-하위 스위치 관계) — 온라인 게임 참여 버튼도 자연히 함께 숨겨짐
- [x] 쿠폰함(`/me/points`)의 "매장 홈페이지 →" 바로가기를 하단 텍스트 링크에서 **업체명 옆 강조 버튼**으로 승격 (유료 기능 노출 강화 목적)
- [x] **가입 승인 게이트 + 만료 시 손님화면 자동 처리** — 신규 가입 매장은 첫 구독료 결제(subscriptions row 등록) 전까지 관리자 진입이 전부 막히고 "승인대기" 화면만 노출(`middleware.ts`가 `trial`/`expired` 상태를 동일하게 차단, 새 상태값 추가 없이 기존 만료 로직에 통합). 구독이 `expired`가 되면 손님 화면(`/play/[storeId]`, `/b/[storeId]`)도 이벤트 조회 자체를 건너뛰어 자연스럽게 "진행중인 이벤트가 없어요"로 전환
- [x] **관리자 메뉴 재구성** — "이벤트 관리"를 최상단 우선 버튼으로 이동, "쿠폰 관리"/"성과 리포트"를 대시보드 하위 탭(개요/쿠폰 현황/성과 리포트, 공통 기간 필터)으로 통합, "계산대"를 "게임 실행"으로 이름 변경 후 손님용 게임 화면을 새 탭으로 직접 연결(계산대 검증(`/staff`)은 작은 보조 링크로 축소). 메뉴 텍스트 한 줄 고정 + 간격 조정
- [x] 모바일 하단바 링크 연결, 홈페이지 서비스/이벤트 CTA 문구·강조 개선

### 2026-09-04 (로고 교체 + 비밀번호 재발급 버그 수정)
- [x] **버그 수정**: 광고주 비밀번호 재발급(`/api/admin/companies/reset-password`)이 `store_accounts` 테이블에 없는 `updated_at` 컬럼을 갱신하려다 항상 실패하던 문제 발견·수정
- [x] `components/BrandLogo.tsx` 신규 — 새 로고 이미지(투명 배경 PNG)를 "단골팅" 워드마크로 렌더링, 랜딩(`Navbar`/푸터)·`/aeo`·관리자 로그인 화면의 기존 텍스트/이모지 로고를 전부 교체. 이미지 잘림(검정 '단골' 글자) 버그 후속 수정

### 2026-09-07 (경품 확률 자동 재조정 + 장기 운영 모드 + 가이드 카테고리)
- [x] `docs/migrations/056_long_term_mode.sql` — `events`에 `long_term_mode`/`reset_cycle`(weekly 또는 monthly)/`current_cycle_start` 추가. **수량 기반(quantity) 모드 이벤트만** 대상으로, 매일 자정(KST) `remaining_quantity` 대비 "실측 일평균 참여자 수"(`activity_log`의 `game_start` 집계, 데이터 없으면 등록 시 입력한 예상치로 폴백)를 기준으로 각 티어의 `computed_probability`를 자동 재조정(`lib/game-engine/dailyRebalance.ts`, `/api/cron/probability-rebalance` 신규 cron). "꽝" 티어도 동일 공식을 그대로 적용해 남는 확률을 자연히 흡수. 장기 운영 모드는 `reset_cycle` 경계(매주 월요일/매달 1일)마다 `remaining_quantity`를 `total_quantity`로 리필 — 이미 발급된 쿠폰(`coupons` 테이블 별도 row)에는 영향 없음. `percent` 모드 이벤트는 배치 대상에서 완전히 제외
- [x] 이벤트 등록/수정 화면에 "장기 운영 모드" 토글 + "매주 리셋/매달 리셋" 라디오 추가(수량 모드에서만 노출)
- [x] 광고주용 확률 자동조정 사용안내 A4 1장 분량 설명문 작성(사장님 요청, 코드 변경 없음)
- [x] 랜딩페이지에 "가이드" 카테고리 신설 (공개 가이드 + 광고주 전용 가이드)

### 2026-09-07 (2차: 카카오 앱 `www.dgting.co.kr` 전용 신규 교체 + 카톡 메시지 버튼/QR 도메인 버그 수정)
- [x] **카카오 앱 신규 생성 및 키 교체** — 기존 앱(구 도메인 시절) 대신 `www.dgting.co.kr` 전용 신규 카카오 앱 생성, `KAKAO_REST_API_KEY`/`NEXT_PUBLIC_KAKAO_JS_KEY`/`KAKAO_CLIENT_SECRET`을 Vercel에 갱신(`NEXT_PUBLIC_` 접두사 변수는 "Secret" 타입으로는 기존 값 수정이 막혀서 삭제 후 "Config" 타입으로 재생성해야 저장됨 — Vercel 정책). 실제 카카오 로그인 리다이렉트(`client_id`/`redirect_uri`)가 새 앱 기준으로 뜨는 것까지 브라우저로 직접 검증
- [x] 카카오 "나에게 보내기" 쿠폰 메시지 버튼 정리 — 버튼1 "매장에서 사용하기"가 게임(랜딩) 페이지로 가던 것을 **내 쿠폰함**(`/me/points?store_id=`)으로 연결 수정, 버튼2 문구를 "당근마켓 후기 남기기"로 정리
- [x] **버그 발견·수정(핵심)**: `NEXT_PUBLIC_APP_URL` Vercel 환경변수가 8/12에 등록된 뒤 한 번도 갱신되지 않아 옛 `roulette-marketing.vercel.app` 값이 그대로 남아있었던 것을 발견 — 매장 고정 QR코드와 카카오 쿠폰 메시지 버튼(`lib/kakao/meMessage.ts`도 동일 환경변수 사용)이 전부 이 값을 우선 사용하고 있어서, 신규 매장 QR을 찍어도 옛 vercel 주소로 인코딩되고 카톡 버튼도 등록되지 않은 도메인이라 카카오 쪽에서 "기본 도메인(루트)"으로 강제 이동시키는 현상이 있었음. Vercel에서 `https://www.dgting.co.kr`로 재등록(Secret→Config 재생성) + 재배포로 해결, 신규 QR 스캔 시 리다이렉트 없이 바로 정식 도메인으로 연결되는 것 확인 완료
- [x] `app/api/admin/store-qr` 응답에 `Cache-Control: no-store` 헤더 + 화면 미리보기 캐시버스터 추가 — 위 버그가 고쳐지기 전에 브라우저가 캐시해둔 옛(vercel.app) QR 이미지를 코드 수정 후에도 계속 재사용하는 사고 방지
...
- [x] **카카오 앱을 `www.dgting.co.kr` 전용으로 신규 생성 완료 (2026-09-07)** — 새 앱 생성 + 콜백 URL 등록 + `KAKAO_REST_API_KEY`/`KAKAO_CLIENT_SECRET`/`NEXT_PUBLIC_KAKAO_JS_KEY` Vercel 갱신 완료, 실제 로그인 리다이렉트 동작 확인됨
...
- [x] **Vercel 프로덕션 `NEXT_PUBLIC_APP_URL` 환경변수 확인·수정 완료 (2026-09-07)** — 실제로 8/12에 등록된 옛 `roulette-marketing.vercel.app` 값이 그대로 남아있던 것을 발견(Secret 타입이라 값이 안 보여서 이번에 처음 확인됨). `https://www.dgting.co.kr`로 재등록(Config 타입) + 재배포 완료, 신규 QR 생성 시 정식 도메인으로 바로 인코딩되는 것 확인. **카카오 쿠폰 메시지 버튼도 같은 환경변수를 쓰므로 같이 고쳐졌을 가능성이 높음 — 다음 세션에서 실제 발송 재확인 필요**
- [ ] **매장 고정 QR코드 실제 인쇄 스캔 테스트 미완료** — 코드는 배포됐으나, 실제 프린터로 인쇄해서 폰 카메라로 스캔했을 때 에러정정 H단계가 코팅지 반사광/오염 환경에서도 잘 읽히는지는 아직 확인 안 함. 추가로 **`NEXT_PUBLIC_APP_URL` 수정 전에 인쇄된 기존 QR 스티커는 옛 vercel 주소가 남아있어 재발행 필요**
- [ ] **카카오 쿠폰 메시지 버튼 재검증 필요** — `lib/kakao/meMessage.ts`도 `NEXT_PUBLIC_APP_URL`을 그대로 쓰기 때문에, 옛 vercel 값이 남아있던 동안 발송된 쿠폰 메시지의 버튼("매장에서 사용하기"/"당근마켓 후기 남기기")이 카카오 쪽 도메인 검증에 걸려 랜딩페이지(기본 도메인 루트)로 강제 이동하는 증상이 있었음. 환경변수 수정으로 자동 해결됐을 가능성이 높지만, 새 카카오 앱의 [제품 링크 관리 > 웹 도메인]에는 `https://www.dgting.co.kr`이 정상 등록되어 있는 것만 확인했고 **버튼 재발송 재테스트는 아직 못 함**
